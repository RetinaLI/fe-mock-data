...
let restartApp = function (app, server, opt) {
...
let startWatchConfig = function (app, server, opt) {
  watch(path.dirname(path.join(path.resolve('.'), opt.path)), { filter: /\.js$/ }, function (evt, name) {
    console.info('file changed: '.blue + name.green);
    key = Date.now();
    restartApp(app, server, opt);
  });
};

let parseRoutes = function (app, serverConfig) {
...
let initStaticPath = function (app, serverConfig) {
...
let start = function (opt = {
  ...
  let app = initExpressApp(opt);
  initStaticPath(app, serverConfig);
  parseRoutes(app, serverConfig);
  return (new Promise((resolve) => {
    if (!opt.app) {
      if (key !== currentKey) return resolve(2);
      let server = app.listen(opt.port, () => {
        resolve();
      });

      if (opt.enableWatch) {
        startWatchConfig(app, server, opt);
      }
    } else {
      resolve();
    }
